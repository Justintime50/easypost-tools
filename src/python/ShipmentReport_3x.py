...
if __name__ == "__main__":
    ...
    while has_more:
        ...
        if 'shipments' in data and len(data['shipments']) > 0:
            ...
            params = ['page_size=100', f'start_datetime={startDate}', f'end_datetime={endDate}', f'before_id={shipments[-1]["id"]}']
        else:
            has_more = False
    ...
    for shipment in shipments:
        ...
        data += [(sign if ('LabelFee' in fees and float(fees['LabelFee']) > 0.0) else '') + (fees['LabelFee'] if 'LabelFee' in fees else ''),
                 (sign if ('PostageFee' in fees and float(fees['PostageFee']) > 0.0) else '') + (fees['PostageFee'] if 'PostageFee' in fees else ''),
                 (sign if ('InsuranceFee' in fees and float(fees['InsuranceFee']) > 0.0) else '') + (fees['InsuranceFee'] if 'InsuranceFee' in fees else '')
        ]

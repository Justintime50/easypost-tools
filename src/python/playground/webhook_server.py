...
class HooksCLI():
    def __init__(self):
        ...
        parser = argparse.ArgumentParser(
            description='A lightweight framework to catch webhooks. Great for testing.'  # noqa
        )
        parser.add_argument(
            '-a',
            '--address',
            default='127.0.0.1',
            type=str,
            help='This is the address Hooks will listen on. (Default: 127.0.0.1)'  # noqa
        )
        parser.add_argument(
            '-p',
            '--port',
...
class Hooks():
    def start_api():
        if DEBUG:
            args = (
                f'Arguments: {ADDRESS}, {PORT}, {LOG}, {DEBUG}'
            )
            logging.debug(args)

...
if __name__ == "__main__":
    ...
    while has_more:
        ...
        if 'shipments' in data and len(data['shipments']) > 0:
            ...
            params = ['page_size=100', f'start_datetime={startDate}',
                      f'end_datetime={endDate}', f'before_id={shipments[-1]["id"]}']
        else:
            has_more = False
    ...
    for shipment in shipments:
        ...
        data += [shipment['customs_info']['id'] if shipment['customs_info'] else '',
        ...
        data += [(sign if ('LabelFee' in fees and float(fees['LabelFee']) > 0.0) else '') + (fees['LabelFee'] if 'LabelFee' in fees else ''),  # noqa
                 (sign if ('PostageFee' in fees and float(fees['PostageFee']) > 0.0)
                  else '') + (fees['PostageFee'] if 'PostageFee' in fees else ''),
                 (sign if ('InsuranceFee' in fees and float(fees['InsuranceFee']) > 0.0)
                  else '') + (fees['InsuranceFee'] if 'InsuranceFee' in fees else '')
                 ]

        sr = shipment['selected_rate']
        data += [shipment['insurance'],
                 sr['id'],
                 sr['carrier'],
                 sr['service'],
                 sr['rate']
        ...
        data += [shipment['is_return'],
                 shipment['tracking_code'],
                 shipment['usps_zone'],
                 shipment['status']
                 ]

        data += [shipment['tracker']['id'] if shipment['tracker'] else '',
                 shipment['tracker']['public_url'] if shipment['tracker'] else '',
                 ]

        data += [shipment['refund_status'],
                 shipment['batch_id'],
                 shipment['batch_status'],
                 shipment['batch_message']
                 ]

        data = [(i if i else '') for i in data]
        rows.append(data)

    cols = ('created_at', 'id', 'reference', 'mode', 'to_address.id', 'from_address.id', 'return_address.id', 'buyer_address.id', 'parcel.id', 'customs_info.id', 'scan_form.id', 'label_fee', 'postage_fee', 'insurance_fee', 'insured_value', 'selected_rate.id',  # noqa
            'selected_rate.carrier', 'selected_rate.service', 'selected_rate.rate', 'postage_label.id', 'postage_label.url', 'is_return', 'tracking_code', 'usps_zone', 'status', 'tracker.id', 'tracker.public_url', 'refund_status', 'batch_id', 'batch_status', 'batch_message')  # noqa
